  ...
  return (
    <div className="flex flex-col items-center gap-3 p-6 bg-gradient-to-br from-primary/8 to-accent/8
      ...
      <div className="w-10 h-10 rounded-2xl bg-primary/15 flex items-center justify-center">
        <Lock size={18} className="text-primary-ink" />
      </div>
      <div>
        <p className="font-display font-bold text-content-primary text-sm mb-1">
          {def?.label ?? 'Pro Feature'}
        </p>
        {def?.description && (
          <p className="text-xs text-content-secondary">{def.description}</p>
        )}
      </div>
      <Link
        href="/settings"

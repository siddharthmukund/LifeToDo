      ...
      <div className="sticky top-0 z-10 bg-gtd-bg/95 backdrop-blur-xl border-b border-white/5 px-5 pt-12 pb-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gtd-muted mt-0.5">{items.length} ideas parked</p>
          </div>
        ...
        <AnimatePresence initial={false}>
          {items.map((item, i) => (
            <motion.div
              ...
              className="bg-gtd-surface border border-white/8 rounded-2xl px-4 py-3
                         flex items-start gap-3"
            >
              <div className="flex-1 min-w-0">
                <p className="text-sm text-gtd-text">{item.text}</p>
                <p className="text-xs text-gtd-muted mt-1">{timeAgo(item.createdAt)}</p>
              </div>
              <div className="flex gap-2 flex-shrink-0 mt-0.5">
                <button
                  onClick={() => handlePromote(item)}
                  className="p-1.5 rounded-lg text-gtd-accent hover:bg-gtd-accent/10 transition-colors"
                  title="Make active"
                >
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg text-gtd-muted hover:text-gtd-danger transition-colors"
                  title="Remove"
                >
                  <Trash2 size={16} />
                </button>
        ...
        <div className="space-y-4">
          <textarea
            ...
            className="w-full bg-gtd-bg border border-white/10 rounded-xl px-4 py-3
                       text-sm text-gtd-text placeholder-gtd-muted focus:outline-none

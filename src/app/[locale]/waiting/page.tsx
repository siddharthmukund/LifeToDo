      ...
      <div className="sticky top-0 z-10 bg-surface-base/95 backdrop-blur-xl border-b border-primary/10 px-6 pt-14 pb-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <Link href="/" className="p-2 rounded-full bg-surface-card border border-border-subtle text-content-secondary hover:text-content-primary active:scale-95 transition-all">
              <ArrowLeft size={18} />
            </Link>
            <div>
              <h1 className="text-2xl font-display font-bold text-content-primary flex items-center gap-2">
                <Clock size={22} className="text-primary-ink fill-primary/20" />
                Waiting For
              </h1>
              <p className="text-[10px] font-bold uppercase tracking-widest text-content-muted mt-0.5">
                ...
                {overdueItems.length > 0 && (
                  <span className="text-status-warning ml-2">· {overdueItems.length} need follow-up</span>
                )}
              </p>
            </div>
          </div>
          ...
          {(['active', 'resolved'] as const).map(t => (
            <button
              ...
              className={`px-5 py-2 rounded-full text-xs font-bold uppercase tracking-widest transition-all active:scale-95
                ${tab === t
                  ? 'bg-primary text-content-inverse shadow-glow-accent'
                  : 'bg-surface-card text-content-muted border border-border-subtle hover:text-content-primary'}`}
            >
              ...
              <>
                ...
                {overdueItems.length > 0 && (
                  <motion.div
                    ...
                    className="flex items-center gap-2 px-4 py-3 bg-status-warning/10 border border-status-warn rounded-2xl mb-2"
                  >
...
function WaitingCard({
  ...
  return (
    ...
    >
      ...
      {/* Task text */}
      <p className="text-base font-bold text-content-primary leading-snug">{item.text}</p>
        ...
        <div className="flex items-center justify-between mt-3">
          <div className="flex items-center gap-2">
            <div className="flex items-center justify-center w-7 h-7 rounded-full bg-primary/15 border border-primary/20 flex-shrink-0">
              <UserCheck size={14} className="text-primary-ink" />
            </div>
            <span className="text-sm font-bold text-content-primary">{item.waitingFor.person}</span>
          </div>
      ...
      {!item.waitingFor && (
        <p className="text-xs text-content-muted mt-2">{timeAgo(new Date(item.updatedAt))}</p>
      )}
      ...
      <div className="mt-4">
        <button
          ...
          className="w-full py-2.5 rounded-xl bg-primary/10 border border-primary/20
                     text-xs font-bold text-primary-ink hover:bg-primary/20 transition-colors active:scale-95"
        >

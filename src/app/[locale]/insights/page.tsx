...
function ChartTooltip({ active, payload, label }: {
  ...
  return (
    <div className="bg-surface-elevated border border-border-default rounded-xl px-3 py-2 text-xs shadow-2xl">
      <p className="text-content-secondary font-bold mb-1">{label}</p>
      {payload.map(p => (
...
export default function InsightsPage() {
  ...
  return (
    <div className="min-h-screen pb-8">
      {/* ── Header ── */}
      <div className="sticky top-0 z-20 bg-surface-base/95 backdrop-blur-xl border-b border-primary/10 px-4 pt-12 pb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <BarChart2 size={20} className="text-primary-ink" />
            <h1 className="text-xl font-display font-bold text-content-primary">Insights</h1>
          </div>
          <button
            onClick={() => void refresh()}
            className="w-8 h-8 rounded-xl bg-overlay-hover flex items-center justify-center hover:bg-overlay-hover transition-colors"
            aria-label="Refresh"
          >
      ...
      <div className="px-4 pt-6 space-y-6">
        ...
        {/* ── Health Score Card ── */}
        <div className="bg-surface-card rounded-2xl border border-border-subtle p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-content-muted mb-1">GTD Health Score</p>
              <p className="text-xs text-content-secondary">How well your system is running</p>
            </div>
            {!score?.hasEnoughData && (
              <span className="text-[10px] font-bold uppercase tracking-widest text-status-warning bg-status-warning/10 px-2 py-1 rounded-lg">
                Warming up
              </span>
            )}
          </div>
          ...
          {score && score.hasEnoughData ? (
                ...
                {(Object.keys(score.breakdown) as (keyof HealthScoreBreakdown)[]).map(key => (
                  <div key={key}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[11px] text-content-secondary">{SCORE_COMPONENT_LABELS[key]}</span>
                      <span className="text-[11px] font-bold text-content-primary">{score.breakdown[key]}%</span>
                    </div>
                    <ProgressBar
            ...
            <div className="flex flex-col items-center gap-3 py-6">
              <Zap size={32} className="text-primary-ink/50" />
        ...
        {!isPro && (
          <div className="bg-gradient-to-br from-primary/10 to-accent/10 rounded-2xl border border-primary/20 p-6 flex flex-col items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-primary/15 flex items-center justify-center">
              <Lock size={20} className="text-primary-ink" />
            </div>
            <div className="text-center">
              <p className="font-display font-bold text-content-primary mb-1">Unlock Detailed Charts</p>
            ...
            <Link
              href="/settings"
              className="bg-primary text-content-inverse font-bold text-sm px-6 py-2.5 rounded-xl hover:bg-primary/90 transition-colors"
            >
              Upgrade to Pro
            </Link>
          </div>
          ...
          <>
            ...
            <div className="bg-surface-card rounded-2xl border border-border-subtle p-5">
              <div className="flex items-center gap-2 mb-4">
                <TrendingUp size={14} className="text-primary-ink" />
            ...
            <div className="bg-surface-card rounded-2xl border border-border-subtle p-5">
              <div className="flex items-center gap-2 mb-4">
                <Inbox size={14} className="text-primary-ink" />
                <p className="text-[10px] font-bold uppercase tracking-widest text-content-muted">
            ...
            {score && score.hasEnoughData && (
              <div className="bg-surface-card rounded-2xl border border-border-subtle p-5">
                <div className="flex items-center gap-2 mb-4">
                  <Activity size={14} className="text-primary-ink" />
                  <p className="text-[10px] font-bold uppercase tracking-widest text-content-muted">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <div key={label} className="bg-surface-card rounded-2xl border border-border-subtle p-4">
              <Icon size={14} className="text-primary-ink mb-2" />
              <p className="text-xl font-display font-bold text-content-primary">{value}</p>
              <p className="text-[10px] text-content-muted uppercase tracking-widest font-bold mt-0.5">{label}</p>
            </div>
          ))}
        </div>

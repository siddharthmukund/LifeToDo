  ...
  return (
    <div className="flex flex-col h-full animate-fade-in pb-8">
      ...
      <header className="flex items-center justify-between px-6 pt-14 pb-6">
        <h1 className="font-display text-2xl font-bold tracking-tight text-content-primary">Inbox</h1>
        <button className="relative rounded-full bg-surface-card p-2 text-primary-ink border border-border-default active:scale-95 transition-transform shadow-glow-accent">
          <Settings2 size={20} />
          {items.length > 0 && <span className="absolute right-2 top-2 size-2 rounded-full bg-primary border-2 border-surface-card"></span>}
        </button>
      </header>
      ...
      <div className="px-6 flex justify-between items-end mb-6">
        <div>
          <p className="text-xs font-bold uppercase tracking-widest text-primary-ink mb-1">Unprocessed</p>
          <h2 className="text-4xl font-display font-bold text-content-primary leading-none mb-3">
            {items.length}
            {items.length > 0 && <span className="text-lg text-content-secondary ml-1">items</span>}
          </h2>
          <XPBar />
        </div>
        {items.length > 0 && (
